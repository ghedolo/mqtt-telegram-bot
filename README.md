...
- **Sessions:** 14 — 4185 messages (1658 user + 2527 assistant)
- **Active conversation time:** ~1095 min (~18h 15m)

*Active time: sum of consecutive gaps ≤ 5 min across all sessions. Longer gaps discarded.*

| Metric | Tokens |
|---|---:|
| Input (non-cache) | 221,331 |
| Output | 1,165,341 |
| Cache write | 4,863,018 |
| Cache read | 176,442,063 |
| **Total** | **~182 M** |

### Caveman mode

All 14 sessions ran with caveman mode active — a Claude Code skill that drops filler words, articles, and pleasantries from assistant responses while keeping full technical content. The assistant produced an average of **461 output tokens per message**. The saving is modest compared to prose-heavy projects because the dominant output here is code, which caveman leaves untouched.
<!-- devstats:end -->
