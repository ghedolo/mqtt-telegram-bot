...
`traceCmd` is separate and orthogonal — it's a *category*, not a severity, so it stays on at any `debug` level. Set `traceCmd: 1` to record an access-log line for every command: who ran it, the exact command text, and how it ended (`ok` with elapsed time, or `FAILED` with the exception). It goes to its own file (`traceCmdFile`, default `data/cmdtrace.log`) and **not** to the main log, so the two never mix. Keep the path under `data/`: in the container `/app` is read-only and only `data/` (the bind-mounted volume, also where the DB lives) is writable — and reachable from the host. What it captures is the command and its outcome, not the reply text the bot sent back — replies leave through many separate calls with no single seam to tap.
