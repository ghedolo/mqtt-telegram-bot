...
devices:
  DEVICE_KEY:
    ...
    hasZigbeeAvailability: true    # optional: trust zigbee2mqtt's online/offline
                                   #   for this device instead of the 3×interval
                                   #   heuristic. Uses `<topic>/availability`.
    availabilityTopic: "…"         # optional: explicit availability topic; use for
                                   #   a custom topic or a device with only per-field
                                   #   topics. Wins over hasZigbeeAvailability.
...
`traceCmd` is separate and orthogonal — it's a *category*, not a severity, so it stays on at any `debug` level. Set `traceCmd: 1` to record an access-log line for every command: who ran it, the exact command text, and how it ended (`ok` with elapsed time, or `FAILED` with the exception). It goes to its own file (`traceCmdFile`, default `data/cmdtrace.log`) and **not** to the main log, so the two never mix. Keep the path under `data/`: in the container `/app` is read-only and only `data/` (the bind-mounted volume, also where the DB lives) is writable — and reachable from the host. A path that can't be opened (read-only or misconfigured) doesn't crash the bot: it logs a warning and starts with the trace off. What it captures is the command and its outcome, not the reply text the bot sent back — replies leave through many separate calls with no single seam to tap.
...
The full permission matrix — how a group name becomes a role on a field, the exact gate on every command, notification fan-out, and the checklist for adding a user — is in [docs/permissions.md](docs/permissions.md).
...
A large part of that suite came from a four-pass review run with a second model
in August 2026 — what it cost, what it found, and what the split of roles did
and did not prove is written up in [docs/second-model-review.md](docs/second-model-review.md).
...
- **Sessions:** 25 — 10752 messages (4046 user + 6706 assistant)
- **Active conversation time:** ~2242 min (~37h 22m)

*Active time: sum of consecutive gaps ≤ 5 min within each session; cumulative and cross-machine.*

| Metric | Tokens |
|---|---:|
| Input (non-cache) | 475,925 |
| Output | 5,279,205 |
| Cache write | 18,483,011 |
| Cache read | 1,065,937,510 |
| **Total** | **~1090 M** |

The assistant averaged **787 output tokens per message**. The early sessions ran with caveman mode — a Claude Code skill that strips filler while keeping full technical content — so this average blends those with later, prose-heavier sessions.
<!-- devstats:end -->
