...
**Threshold** — an alarm value set per-Field. A Reading above (or below) the Threshold triggers an Alarm. Keyed by Sensor name in the DB — it is *state*, not configuration, and survives a config change. Two ways in: an Admin sets it with `/setAlarm`/`/setAlarmLow`, or Sensor Config **seeds** it from `defaultAlarmHigh`/`defaultAlarmLow` at startup and on `/reloadConfig`. Seeding applies **only when no Threshold is set**, so a seeded Field arms itself on a fresh deployment without anyone typing a command, and a later `/setAlarm` is never overwritten. The converse: `/clearAlarm` on a seeded Field is undone by the next reload — to clear it for good, drop the key from Sensor Config too.
...
**Daily Digest** — a scheduled silent message sent once per day at a configurable time (`digest_time` in credentials config, default `15:00`). Per-user: only Fields the User has subscribed to via `/digest` and can see. Format: the same monospace `Sensor | value | min ago` table `/get` renders (`_render_sensors_text`), so the two never drift apart — including the `∞` a Sensor gets once it counts as silent. A Sensor with no stored Reading shows `-`.

## Notification behaviour

- Threshold alarm messages sent via DM to **Viewers** of the affected Field — which includes its Admins, since Admin implies Viewer. Read access to a Field and receipt of its threshold alarms are the same right: there is no way to see a Field without being told when it crosses.
- Offline alarm messages sent via DM to Admins of Fields for which the User has an active digest subscription in that Device. Both conditions are required: an Admin who never subscribed to any Field of a Device is not told that Device went offline. So offline has a *narrower* fan-out than threshold, and one that a User opts into.
