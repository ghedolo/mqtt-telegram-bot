...
- `test_a_single_low_reading_is_not_enough_for_a_verdict` / `test_enough_low_readings_turn_the_same_shape_into_a_floor` — the production run found **one** reading below `below` in 78k samples, and one sample is always a "tight cluster": under three low readings the tool reports insufficient evidence instead of a floor.
- `test_a_reading_inside_a_window_is_at_distance_zero` / `test_the_sample_after_a_short_outage_is_measured_from_its_end` / `test_distance_is_taken_to_the_nearest_of_several_windows` / `test_an_open_window_runs_to_now` / `test_no_recorded_window_yields_no_distance` — a 62 s meter takes no sample inside a 5 s outage, so "outside every window" alone would have counted the outage's own tail sample as proof that power was present; readings are attributed to a window by distance, within a margin of two publish intervals.
...
- `test_silent_unmute_reports_only_real_mutes` / `test_silent_unmute_nothing_muted` / `test_silent_unmute_expired_mute_is_not_counted` — the unmute reply counts the mutes `db.unmute_sensor` actually removed, not the fields the expr matched: `/silent DEI*` used to answer `Unmuted 4 field(s)` right after `/silent` had answered `No active mutes`. An expired row is dropped but not counted, so the two commands agree.
...
- `test_list_renders_one_block_per_device` / `test_list_thresholds_marked_by_direction` / `test_list_field_without_reading_still_listed` — the block layout: a code fence (without it the columns collapse in Telegram), the device key on its own line, fields indented under it, `△`/`▽` for the two threshold bands, `--` for a field that has never been read.
- `test_list_footer_escapes_the_sensor_name_example` — the message is Markdown (the code fence needs it), so the footer's underscores must be escaped: `SM2_UTA1_T` reached Telegram as italics and arrived as `SM2UTA1T`, a name nobody can type back. Blackout group ids and their `info` are escaped the same way.
- `test_list_splits_long_listing_and_closes_every_fence` — a listing past the 4096-char limit is split, and every chunk opens and closes its own fence: splitting mid-block used to leave a fence open, so Telegram rendered the rest of that message as plain text.
...
- `test_last5alarm_blackout_group` / `..._blackout_group_hidden_from_outsider` — `/last5Alarm` resolves a Blackout Group id case-insensitively (it is an alarm subject like a Sensor name is) and refuses one the caller may not view, with the same "unknown" reply an unknown sensor gets, so the id itself never leaks.
