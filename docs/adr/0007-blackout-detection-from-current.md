...
- **Config.** A `blackouts:` block, allowed only in `00-defaults.yaml` (the sole file permitted non-`devices` top-level keys). It is a map of Blackout Group id → rule: `fields` (canonical Sensor names to watch), `below` (amps; all must be under it), `for_seconds` (sustained duration before raising), `repeat_seconds` (re-notify interval, default = `alarm_offline_repeat`). Startup validation hard-errors on unknown Field names.
- **Evaluation.** In `AlarmManager`'s periodic offline-check loop, a blackout check reads the latest value of each watched Field; if all are below `below` for longer than `for_seconds`, it raises a blackout Alarm keyed by the group id, repeating no more often than `repeat_seconds`. It auto-clears (with a recovery message) when any watched Field rises above the threshold.
