...
- **Evaluation.** Event-driven: `check_blackout_for(sensor)` runs from `on_reading` on every incoming current reading, so detection latency is roughly the meter's publish cadence (not tied to the 60 s offline loop, which brief outages would outrun). A watched Field is classified from its latest reading as **DARK** (fresh — newer than `stale_after` — and below `below`), **LIT** (fresh and at/above `below`), or **UNKNOWN** (stale/missing). A blackout Alarm keyed by the group id is raised when all Fields are DARK, sustained for `for_seconds`, repeating no more often than `repeat_seconds`. It ends (recovery message) **only on positive proof** — when a Field becomes LIT; a Field going UNKNOWN does not end it (the alarm is held and the Field's device offline alarm reports the silence). This avoids a false recovery when, during a real outage, one current meter dies while another still reads zero — the earlier version cleared on staleness and emitted a spurious "power restored". **`for_seconds` (sustain) and `stale_after` (freshness) are decoupled** — an earlier version conflated them into one, which made a small `for_seconds` silently unobservable whenever it fell below the meter's publish interval.
